...
int ballMoveX = 1, ballMoveY = 0, ballPosY = 0, ballPosX = 0;
...
void moveBall() {
  ...
  if(ballPosX == 0) checkLeftSide();
  else if (ballPosX == 19) checkRightSide();

  // Regarde les collisions avec les murs du jeu
  if(ballPosY == 0) Serial.println("We are on the bottom of the game board");
  else if (ballPosY == 19) Serial.println("We are on the top of the game board");
...
void checkLeftSide() {
  ...
  if(index = -1) {
    ...
    Serial.println("The first player lose");
  }
}
...
void checkRightSide() {
  ...
  if(index != -1) {
    ...
    Serial.println("The second player lose");
  }
}
...
void moveSecondPlayer() {
  ...
  if(move < 0) { // Si le joueur déplace sa planche en haut
    if(boardSecondPlayer[4] == 19) return; // Si la planche est au plus haut
    for(int i = 4; i > 0; i--) boardSecondPlayer[i] = boardSecondPlayer[i + 1];
    boardSecondPlayer[4]++;
  ...
  for(int i = 0; i < 20; i++) {
    for(int j = 0; j < 20; j++) {
      if(leds_tab[i][j].red != 0 || leds_tab[i][j].green != 0 || leds_tab[i][j].blue != 0) {
        pixels.setPixelColor(leds_tab[i][j].number, pixels.Color(leds_tab[i][j].red, leds_tab[i][j].green, leds_tab[i][j].blue));
      }
    }
  }
  pixels.show();
